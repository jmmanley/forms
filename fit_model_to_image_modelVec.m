function [shapevars,result] = fit_model_to_image_modelVec(project, model, image, parameters)
...
%                     end
    ...
    function W = jacobmult(Jdata, X, flag)
        ...
        if flag > 0
            ...
            Y(es(2) + 1:es(2) + 3 * P, :) = ...
                Y(es(2) + 1:es(2) + 3 * P, :) ...
                + (parameters.xi_0) * tplatesqrt_3 * ...
                vars(vs(2) + 1:vs(2) + 3 * P)' * X(sv, :);
            for m = 1:cM
                ms = es(2) + 1 +       m * 3 * P;
                me = es(2) +     (m + 1) * 3 * P;
                Y(ms:me, :) = Y(ms:me, :) + tplatesqrt_3 * ...
                    (parameters.xi_def) * vars(vs(2) + 1 + m * 3 * P:...
                    vs(2) + (m + 1) * 3 * P)' ...
                    * X(sv, :);
            end
