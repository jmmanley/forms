function [model, modelVec, history] = forms_saveIntermediateSteps(project, parameters)
...
% Saves intermediate steps in "history". Use plot_modelfitting_video.m to
% visualize.
%
...
% Iteration number (for saving purposes)
iterNum = 1;
...
% Re-organize history
history = struct();
history.numIter   = iterNum-1;
history.modes     = modesHistory;
history.shapevars = shapevarsHistory;
history.rotate    = rotateHistory;
history.translate = translateHistory;
history.scale     = scaleHistory;
...
% where E is the energy described in the paper.
    function [energy Jdata] = calc_energy(vars)
        ...
        modesHistory{iterNum}     = modes;
        shapevarsHistory{iterNum} = [];
        rotateHistory{iterNum} = {};
        translateHistory{iterNum} = {};
        scaleHistory{iterNum} = [];
        for i=1:n
            v   = vars(rvs(i):rve(i));
            shapevarsHistory{iterNum}(:,i) = vars(mvs(i):mve(i));
            rotateHistory{iterNum}{i}      = ...
                [ expm([    0 -v(3)  v(2) ;
                v(3)     0 -v(1) ;
                -v(2)  v(1)     0 ]) zeros(3, 1) ;
                zeros(1, 3) 1 ];
            
            scaleHistory{iterNum}(i)       = vars(sv(i));
            
            translateHistory{iterNum}{i}   = ...
                [ 1 0 0 vars(tvs(i)    ) ;
                0 1 0 vars(tvs(i) + 1) ;
                0 0 1 vars(tvs(i) + 2) ;
                0 0 0 1                ];
